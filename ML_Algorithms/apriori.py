...
def generate_recommendations():
    print("Processing new recommendations...")
    ...
    frequent_itemsets = apriori(df_encoded, min_support=0.1, use_colnames=True)  # Adjusted min_support
    ...
    print(f"Recommendations updated successfully and saved to:\n1. {json_file_1}\n2. {json_file_2}")

# Watchdog event handler
class ExcelFileHandler(FileSystemEventHandler):
    def on_modified(self, event):
        if event.src_path.endswith(excel_file):
            print(f"Detected change in {excel_file}, regenerating recommendations...")
...
def monitor_file():
    ...
    observer.start()
    print(f"Monitoring {excel_file} for changes...")
...
if __name__ == "__main__":
    generate_recommendations()  # Generate recommendations initially
    monitor_file()  # Start monitoring the file
